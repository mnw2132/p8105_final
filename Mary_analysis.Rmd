...
```
The findings from the table demonstrate that the lyme rate and tick population density are slightly positively and significantly associated with B. burgdorferi percent. However, this does not appear to be related to the number of ticks collected considering that the number did not have a significant impact on the B. burgdorferi percent.
...
```{r}
# Step 1: Boxplot of 'cases' per 'year'

data =
  read_csv("Data Preparation/final_data.csv")
  
ggplot(data, aes(x = factor(year), y = cases)) +
  geom_boxplot() +
  labs(
    title = "Distribution of Cases by Year",
    x = "Year",
    y = "Cases"
  ) +
  theme_minimal()
...
ggplot(data, aes(x = avg_temperature)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, fill = "blue", alpha = 0.7, color = "black") +
...
ggplot(data, aes(x = prev_badtick)) +
...
# Step 5: Regression model of case_count on prev_badtick controlling for avg_temperature
model <- lm(case_count ~ prev_badtick + avg_temperature, data = data)
summary(model)

# Step 6: Create a summary statistics table (Table 1) for each county
table1 <- data %>%
  select(county, cases, avg_temperature, prev_badtick) %>%
  tbl_summary(
    by = county,
    statistic = list(
