...
```{r}
lyme_rate_vs_area <- lm(lyme_rate ~ shape_area  + shape_leng + pop2020, data = tick_lyme_weather)
...
```{r}
lyme_rate_vs_area_human <- lm(lyme_rate ~ shape_area  + shape_leng  + pop2020, data = tick_lyme_weather)
...
tick_lyme_weather_log <- tick_lyme_weather |>
  mutate(
    ...
    log_lyme_rate = log1p(lyme_rate)
  )

# Check for linear relationship with a scatterplot
ggplot(tick_lyme_weather_log, aes(x = log_tick_density, y = log_lyme_rate)) +
  geom_point() +
...
log_model <- lm(log_lyme_rate ~ log_total_ticks + log_tick_density + b_burgdorferi_percent, data = tick_lyme_weather_log)
summary(log_model)
