---
title: "Data Visuals 2022 + Heat Map"
...
```{r}
#Calculate the correlation coefficient between avg temperature and avg lyme rate
correlation <- cor(tick_lyme_weather_summary$avg_temp, tick_lyme_weather_summary$avg_lyme_rate, use = "complete.obs")
print(paste("Correlation between average temperature and Lyme rate:", correlation))

#The correlation coefficient is -0.24 meaning as average temperature increases, there is a slight tendency for the average Lyme rate to decrease, and vice versa. However, the weak magnitude implies that this pattern is not very pronounced
```

```{r}
#Create visual plot for correlation between average temperature and lyme disease rate
ggplot(tick_lyme_weather_summary, aes(x = avg_temp, y = avg_lyme_rate)) +
  geom_point(color = "blue", size = 2) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(
    title = "Correlation between Avg Temperature and Lyme Disease Rate",
    x = "Average Temperature (°F)",
    y = "Average Lyme Disease Rate"
  ) +
  theme_minimal()
```

```{r}
#Create heat map of average temperature by county
ggplot(tick_lyme_weather_summary) +
  geom_sf(aes(fill = avg_temp, geometry = geometry), color = "white") +
  scale_fill_viridis_c(option = "C") +
  labs(
    title = "Heat Map of Average Temperature by County",
    fill = "Avg Temp (°F)"
  ) +
  theme_minimal()
#Looking at this heat map, we can observe than as me move from North to South, the temperatures increase around 12 degrees. The avg temp for most counties ranges between 46 to 52 degrees.
```
